...
The servo used in the car is \textit{EMAX} ES3001. It has plastic gears and operating voltage between \SI{4.8}{\V} to \SI{6.0}{\V}. Speed and torque depend on the supply voltage. According to the product specifications \cite{emax}, the servo can achieve an operating speed of $0.17\, \text{s}/60^\circ$ and torque of \SI[inter-unit-product =\ensuremath{\cdot}]{3.2}{\kg\cm} with the \SI{4.8}{\V} supply voltage. From the operating speed we can calculate angular velocity: 
\begin{equation}
\omega = \frac{d \theta}{d t} = \frac{\frac{\pi}{3}}{0.17} \doteq 6.156\ \unit{\radian\per\second}\ ,
\end{equation}
...
A digital thermometer DS18B20 provides temperature measurement. The sensor is manufactured by \textit{Dallas Semiconductor}, which \textit{Maxim Integrated} later acquired. It can measure temperatures from \SI{-55}{\degreeCelsius} to \SI{125}{\degreeCelsius}. It has a programmable resolution ranging from 9 to 12 bit and $\pm$\SI{0.5}{\degreeCelsius} accuracy over most of the measuring range. The sensor uses a 1-Wire interface, which requires only one pin for communication. When combined with parasitic power mode, the whole thermometer requires only two pins for operation \cite{ds_datasheet}.
