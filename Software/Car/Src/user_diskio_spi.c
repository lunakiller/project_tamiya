...
#define FCLK_FAST() { MODIFY_REG(SD_SPI_HANDLE.Instance->CR1, SPI_BAUDRATEPRESCALER_256, 4); }	/* Set SCLK = fast, approx 9 MBits/s */
																																// CHANGED
...
)
{
	...
	// }
	while(HAL_SPI_GetState(&SD_SPI_HANDLE) != HAL_SPI_STATE_READY);
	HAL_SPI_Transmit(&SD_SPI_HANDLE, (uint8_t*)buff, btx, 50);
}
#endif
